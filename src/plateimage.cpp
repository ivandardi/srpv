...
namespace
{
	                       ...
	                       int morph_op)
	{
		...
		// Wolf
		int k = 0, win = 18;
		NiblackSauvolaWolfJolion(img_gray, thresh, NiblackVersion::WOLFJOLION,
		                         win, win, 0.05 + (k * 0.35));
	...
	std::vector<size_t> dbscan_region_query(
				...
				if (neighbors.size() >= min_pts) {
					...
					for (size_t j : neighbors) {
						if (!visited[j]) {
							visited[j] = true;
							auto jneighbors =
							    dbscan_region_query(points, j, eps);
	...
	void filter_small_rects(std::vector<cv::Rect> &chars,
	                        int min_area,
	                        int max_area)
	{
	...
	std::vector<std::vector<cv::Rect>> filter_dbscan(
	...
	cv::Rect minimum_bounding_rectangle(const std::vector<cv::Rect> &chars,
	                                    cv::Size &&max_size)
	{
		cv::Rect char_roi =
		    std::accumulate(begin(chars), end(chars), chars.front(),
		                    std::bit_or<cv::Rect>());
	...
	cv::Mat unwarp_characters(const cv::Mat &image_preprocessed,
	                          const std::vector<cv::Rect> &chars)
	{
		auto rects =
		    std::minmax_element(begin(chars), end(chars),
		...
		auto desired_height =
		    std::max(rects.first->height, rects.second->height);
		...
		cv::warpPerspective(image_preprocessed, image_transformed,
		                    transformation, {desired_width, desired_height});
