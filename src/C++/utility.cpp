...
Mat drawImageDashboard(vector <Mat> images, int imageType,
                       unsigned int numColumns) {
	unsigned int numRows = ceil((float) images.size() / (float) numColumns);

	Mat dashboard(Size(images[0].cols * numColumns, images[0].rows * numRows),
	              imageType);

	for (unsigned int i = 0; i < numColumns * numRows; i++) {
		if (i < images.size())
			images[i].copyTo(dashboard(Rect((i % numColumns) * images[i].cols,
			                                floor((float) i / numColumns) *
			                                images[i].rows, images[i].cols,
			                                images[i].rows)));
		else {
			Mat black = Mat::zeros(images[0].size(), imageType);
			black.copyTo(dashboard(Rect((i % numColumns) * images[0].cols,
			                            floor((float) i / numColumns) *
...
Mat addLabel(Mat input, string label) {
	...
	Mat newImage(
	Size(input.cols + (border_size), input.rows + extraHeight + (border_size)),
	input.type());
	...
	rectangle(newImage, Point(0, 0), Point(input.cols, extraHeight), bg,
	          CV_FILLED);
	putText(newImage, label, Point(5, extraHeight - 5), CV_FONT_HERSHEY_PLAIN,
	        0.7, fg);

	rectangle(newImage, Point(0, 0),
	          Point(newImage.cols - 1, newImage.rows - 1), border_color,
	          border_size);
...
double median(int array[], int arraySize) {
	...
	return arraySize % 2 ? array[arraySize / 2] :
	       (array[arraySize / 2 - 1] + array[arraySize / 2]) / 2;
}
...
cv::Point findClosestPoint(cv::Point2f* polygon_points, int num_points,
                           cv::Point position) {
	...
	return Point((int) polygon_points[closest_point_index].x,
	             (int) polygon_points[closest_point_index].y);
}

std::vector <cv::Point>
sortPolygonPoints(cv::Point2f* polygon_points, cv::Size surrounding_image) {

	vector <Point> return_points;

	// Find top-left
	return_points.push_back(findClosestPoint(polygon_points, 4, Point(0, 0)));
	return_points.push_back(
	findClosestPoint(polygon_points, 4, Point(surrounding_image.width, 0)));
	return_points.push_back(findClosestPoint(polygon_points, 4,
	                                         Point(surrounding_image.width,
	                                               surrounding_image.height)));
	return_points.push_back(
	findClosestPoint(polygon_points, 4, Point(0, surrounding_image.height)));
...
float getContourAreaPercentInsideMask(cv::Mat mask,
