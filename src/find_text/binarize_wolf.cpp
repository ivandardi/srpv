...
static double
calcLocalStats(const cv::Mat& im, cv::Mat& map_m, cv::Mat& map_s, int winx, int winy)
{
...
namespace srpv {
void NiblackSauvolaWolfJolion(const cv::Mat& im, cv::Mat& output, NiblackVersion version, int winx, int winy, double k, double dR)
{
    double m, s, max_s;
    double th = 0;
    double min_I, max_I;
    ...
    for (int y = 0; y < im.rows; ++y) {
        for (int x = 0; x < im.cols; ++x) {
            output.at<unsigned char>(y, x) = (im.at<unsigned char>(y, x) >= thsurf.at<float>(y, x)) ? 255 : 0;
        }
    }
}
}
